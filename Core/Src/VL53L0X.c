...
void writeReg(uint8_t reg, uint8_t value) {
	...
	i2cStat = HAL_I2C_Mem_Write(&VL53L0X_I2C_Handler, g_i2cAddr | I2C_WRITE,
			reg, 1, msgBuffer, 1, I2C_TIMEOUT);
}
...
uint8_t readReg(uint8_t reg) {
	...
	i2cStat = HAL_I2C_Mem_Read(&VL53L0X_I2C_Handler, g_i2cAddr | I2C_READ, reg,
			1, msgBuffer, 1, I2C_TIMEOUT);
	value = msgBuffer[0];
...
void writeMulti(uint8_t reg, uint8_t const *src, uint8_t count) {
	...
	i2cStat = HAL_I2C_Mem_Write(&VL53L0X_I2C_Handler, g_i2cAddr | I2C_WRITE,
			reg, 1, msgBuffer, count, I2C_TIMEOUT);
}
...
void getSequenceStepTimeouts(SequenceStepEnables const *enables,
	...
	timeouts->final_range_mclks = decodeTimeout(
			readReg16Bit(FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI));
